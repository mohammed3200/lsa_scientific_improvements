"""
LSA Legacy Encryption (Phase 3) — 5-Round Hybrid Block Cipher.
Original paper: Mahlake et al., Journal of Communications, Vol. 18, No. 1, 2023
...
def lsa_encrypt(plaintext: int, round_keys: Tuple[int, ...]) -> int:
    """
    Encrypt a 64-bit plaintext block using LSA Legacy (5 rounds).
...
def lsa_decrypt(ciphertext: int, round_keys: Tuple[int, ...]) -> int:
    """
    Decrypt a 64-bit ciphertext block using LSA Legacy.
    Uses round keys in reverse order.
    """
